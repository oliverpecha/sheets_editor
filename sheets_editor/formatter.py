...
class SheetFormatter:
    ...
    def _get_cell_format(self, sheet_id, row_index, start_col, end_col):
        """
        Helper method to get the format of a range of cells.
        """
        # Make an API call to retrieve the cell format
        response = self.spreadsheet.spreadsheets().get(
            spreadsheetId=sheet_id,
            ranges=f"Sheet1!{self._get_column_letter(start_col)}{row_index + 1}:{self._get_column_letter(end_col)}",
            fields="sheets/data/rowData/values/userEnteredFormat"
        ).execute()
    
        # Parse the response to extract the existing format
        if 'sheets' in response and len(response['sheets']) > 0:
            sheet = response['sheets'][0]
            if 'data' in sheet and len(sheet['data']) > 0:
                row_data = sheet['data'][0]
                if 'rowData' in row_data and len(row_data['rowData']) > 0:
                    row = row_data['rowData'][0]
                    if 'values' in row and len(row['values']) > 0:
                        cell_format = row['values'][0].get('userEnteredFormat', {})
                        return cell_format
        return {}
