...
class SheetFormatter:
        ...
        for cond_format in conditional_formats:
            ...
            for i, row in enumerate(values[1:], 1):  # Start from the second row (data rows)
                if formatting_type == 'case_specific':
                    ...
                    self._apply_all_conditions_formatting(requests, i, row, conditions, cond_format, header, num_cols)
    ...
    def _apply_all_conditions_formatting(self, requests, row_index, row, conditions, cond_format, header, num_cols):
